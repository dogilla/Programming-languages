...
(define (caseif lista)
  (let ([x (car lista)])
    (cond
    [(condition? (car lista)) (if (condition-test-expr (car lista)) (condition-then-expr (car lista)) (caseif (cdr lista)))]
    [(else-cond? (car lista)) (else-cond-else-expr (car lista))])))
...
(define (desugar expr)
  (type-case SCFWBAE expr
    ...
    [withS (f body) (app (fun (map (λ (v) (binding-id v)) f) (desugar body)) (map desugar (map (λ (v) (binding-value v)) f)))]
    [withS* (f body) (cond
